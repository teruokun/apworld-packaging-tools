...
"""Validate Island manifest and package structure."""
...
@click.option(
    ...
    help="Path to island.json manifest to validate.",
)
@click.option(
    "--pyproject",
    "-p",
...
def validate(
    ...
    Validates the island.json manifest against the schema, checks version
    ...
    if pyproject is not None:
        ...
        if pyproject_data:
            ...
            # Check tool.island section
            tool_island = pyproject_data.get("tool", {}).get("island", {})
            if not tool_island:
                warnings.append("No [tool.island] section found. Using defaults.")
