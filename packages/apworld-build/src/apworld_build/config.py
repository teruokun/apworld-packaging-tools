...
"""Build configuration for APWorld packages.

This module provides the BuildConfig dataclass that holds all configuration
needed to build APWorld distributions.
"""
...
# Default schema version for archipelago.json
...
@dataclass
class BuildConfig:
    """Configuration for building APWorld distributions.
    ...
    def from_pyproject_dict(
        ...
        tool_apworld_build = tool_apworld.get("build", {})
        tool_apworld_vendor = tool_apworld.get("vendor", {})
        ...
        # Game name: from tool.apworld.game or derived from name
        game_name = tool_apworld.get("game")
        if not game_name:
        ...
        # Build patterns
        include_patterns = tool_apworld_build.get("include", DEFAULT_INCLUDE_PATTERNS.copy())
        exclude_patterns = tool_apworld_build.get("exclude", DEFAULT_EXCLUDE_PATTERNS.copy())

        # Vendor config
        vendor_exclude = tool_apworld_vendor.get("exclude", [])
        ...
        return cls(
            ...
            minimum_ap_version=tool_apworld.get("minimum_ap_version", ""),
            maximum_ap_version=tool_apworld.get("maximum_ap_version", ""),
            platforms=tool_apworld.get("platforms", []),
            schema_version=tool_apworld.get("schema_version", DEFAULT_SCHEMA_VERSION),
            compatible_version=tool_apworld.get("compatible_version", MIN_COMPATIBLE_VERSION),
    ...
    def from_manifest(
        ...
        """Create a BuildConfig from an archipelago.json manifest (legacy mode).

        Args:
            manifest_path: Path to archipelago.json
            source_dir: Source directory
        ...
        if not path.exists():
            raise FileNotFoundError(f"archipelago.json not found: {path}")
    ...
    def to_manifest(self) -> dict[str, Any]:
        """Convert the config to an archipelago.json manifest dictionary.
