...
Repository server for Island package hosting and discovery.
...
```python
from island_api import create_app, APIConfig
...
```python
from island_api import APIConfig, DatabaseConfig, StorageConfig, RateLimitConfig, AuthConfig

config = APIConfig(
    # Server settings
    title="Island Package Index",
    description="Repository server for Island packages",
    version="0.1.0",
    debug=False,
...
```python
from island_api import DatabaseConfig

database = DatabaseConfig(
    url="sqlite:///./island_repository.db",  # Database URL
...
```python
from island_api import StorageConfig
...
```python
from island_api import RateLimitConfig
...
```python
from island_api import AuthConfig
...
```bash
# Database
ISLAND_DATABASE_URL=postgresql://localhost/island
ISLAND_DATABASE_ECHO=false

# Storage
ISLAND_STORAGE_BACKEND=s3
ISLAND_STORAGE_LOCAL_PATH=./packages
ISLAND_STORAGE_S3_BUCKET=my-bucket

# Rate limiting
ISLAND_RATE_LIMIT_ENABLED=true
ISLAND_RATE_LIMIT_RPM=100

# Authentication
ISLAND_OIDC_ENABLED=true
ISLAND_OIDC_ISSUER=https://github.com
...
hash_value = compute_sha256_file("path/to/file.island")

# Compute SHA256 from stream
with open("file.island", "rb") as f:
...
verify_checksum_file("file.island", expected_hash)
```
...
curl -X POST https://api.example.com/v1/packages/my-game/upload \
  ...
  -F "file=@my_game-1.0.0-py3-none-any.island"
```
...
```yaml
- name: Publish to Island Repository
  uses: actions/upload-artifact@v4
  env:
    ISLAND_OIDC_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```
...
```bash
docker build -t island-api .
docker run -p 8000:8000 \
  -e ISLAND_DATABASE_URL=postgresql://db/island \
  -e ISLAND_STORAGE_BACKEND=s3 \
  -e ISLAND_STORAGE_S3_BUCKET=my-bucket \
  island-api
```
...
# Using Python
from island_api import create_app
import json
