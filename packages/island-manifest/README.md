...
```console
pip install island-manifest
```

## Features

- JSON Schema validation for `island.json`
- Transform `pyproject.toml` to `island.json`
- Support for PEP 621 project metadata
- Island-specific configuration via `[tool.island]`
- Backward compatible with existing Island manifests
...
Validate an island.json manifest against the schema.

```python
from island_manifest import validate_manifest
...
```python
from island_manifest import validate_manifest_strict, ManifestValidationError
...
Transform a pyproject.toml file into an island.json manifest.

```python
from island_manifest import transform_pyproject
...
```python
from island_manifest import transform_pyproject_dict
...
```python
from island_manifest import TransformConfig, transform_pyproject

config = TransformConfig(
    schema_version=7,      # Island schema version
...
The `island.json` manifest follows this schema:
...
| `version` | integer | Island schema version (must be 7) |
...
Configure your Island package using standard PEP 621 metadata plus `[tool.island]`:
...
| pyproject.toml | island.json |
|----------------|-------------|
| `project.name` | `game` (title-cased if `tool.island.game` not set) |
...
| `tool.island.game` | `game` |
| `tool.island.minimum_ap_version` | `minimum_ap_version` |
| `tool.island.maximum_ap_version` | `maximum_ap_version` |
| `tool.island.platforms` | `platforms` |
| `tool.island.pure_python` | `pure_python` |
