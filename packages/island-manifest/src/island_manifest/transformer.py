...
"""Transform pyproject.toml to island.json manifest.

This module parses PEP 621 project metadata and Island-specific configuration
from pyproject.toml and generates the runtime island.json manifest.
"""
...
class TransformConfig:
    ...
    Attributes:
        schema_version: Island schema version to use
...
def transform_pyproject(
    ...
    This function reads the [project] section (PEP 621) and [tool.island] section
    from pyproject.toml and generates a valid island.json manifest.
    ...
    Returns:
        A dictionary containing the island.json manifest
    ...
    # Extract game name (required) - from tool.island or project.name
    game = tool_island.get("game")
    if not game:
        ...
        if project_name:
            ...
            raise ManifestTransformError(
                "Missing required field: 'game' in [tool.island] or 'name' in [project]"
            )
    ...
    # Add AP version compatibility from tool.island
    min_ap = tool_island.get("minimum_ap_version")
    if min_ap:
        manifest["minimum_ap_version"] = min_ap

    max_ap = tool_island.get("maximum_ap_version")
    if max_ap:
    ...
    # Add platform info from tool.island
    platforms = tool_island.get("platforms")
    if platforms:
        manifest["platforms"] = platforms

    # Add pure_python flag from tool.island
    pure_python = tool_island.get("pure_python")
    if pure_python is not None:
        manifest["pure_python"] = pure_python

    # Extract entry points from [project.entry-points.ap-island]
    entry_points = project.get("entry-points", {})
    ap_island_eps = entry_points.get("ap-island", {})
    if ap_island_eps:
        manifest["entry_points"] = {"ap-island": ap_island_eps}
...
def transform_pyproject_dict(
    ...
    """Transform a pyproject.toml dictionary into an island.json manifest.
